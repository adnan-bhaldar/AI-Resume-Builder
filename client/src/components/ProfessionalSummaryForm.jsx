...
const generateSummary = async () => {
    try {
        ...
        const newSummary = response.data.enhancedContent;

        if (newSummary) {
            // 1. Update the parent's state object (for saving to DB)
            setResumeData(prev => ({ ...prev, professionalSummary: newSummary }));
            
            // 2. IMPORTANT: Update the local input value immediately
            // This is what actually changes the text in your textarea
            onChange(newSummary); 
